...
export default function EditPurchasePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [purchase, v, p] = await Promise.all([
        api.getPurchase(parseInt(id, 10)),
        api.listVendors(),
        api.listProducts(),
      ]);
      ...
      if (purchase.purchase_payments && purchase.purchase_payments.length > 0) {
        const payment = purchase.purchase_payments[0];
        setPaidAmount(payment.amount || "");
        setPaymentMethod(payment.payment_method || "cash");
        setPaymentNote(payment.note || "");
      } else if (purchase.paid_total) {
        setPaidAmount(purchase.paid_total);
      }
  ...
  async function updatePurchase() {
    if (!canUpdate || !vendorId || !id) return;
    setError(null);
    setLoading(true);
    ...
    try {
      await api.updatePurchase(parseInt(id, 10), {
      ...
      setError(e instanceof Error ? e.message : "Update purchase failed");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="grid gap-3">
      <div className="card">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 md:mb-5">
          <div>
            <h3 className="m-0 mb-1 text-xl sm:text-2xl font-semibold">Edit Purchase</h3>
            <div className="text-xs text-gray-600">Update purchase information</div>
          </div>
        ...
        <button
          className={`btn btn-primary mt-4 w-full sm:w-auto ${canUpdate ? "" : "opacity-50"}`}
          disabled={!canUpdate || loading}
          onClick={onUpdate}
        >
          {loading ? "Updating Purchase..." : "Update Purchase"}
        </button>
        <Link to="/purchases" className="btn mt-2">
          Cancel
        </Link>
      </div>
